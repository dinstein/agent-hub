...
// ListTools returns the governance state of every known tool; a non-empty
// server narrows the listing.
func (h *Hub) ListTools(ctx context.Context, server string) (api.ToolList, error) {
	return call(ctx, h, func(c *api.Client) (api.ToolList, error) {
		return c.Tools.List(ctx, server)
	})
}

// SetToolEnabled flips one tool's kill switch.
//
// It works with the server DOWN — an operator must be able to disable a
// suspicious tool without first starting the thing that serves it. The
// switch is orthogonal to the integrity approval: switching a tool off and
// back on never discards an approval and never grants one.
//
// Precondition caveat (api.ToolsService): these stores are not the registry,
// so expectedGeneration is honored as an advisory staleness check, not as a
// compare-and-swap on them.
func (h *Hub) SetToolEnabled(
	ctx context.Context, server, tool string, enabled bool, expectedGeneration uint64,
) (api.ToolWrite, error) {
	return call(ctx, h, func(c *api.Client) (api.ToolWrite, error) {
		return c.Tools.SetEnabled(ctx, server, tool, enabled, expectedGeneration)
	})
}

// SetToolOverride edits one tool's local presentation, or clears it.
//
// The description override is the NEUTRALIZATION path for a prompt-injection
// carrier: the downstream keeps its poisoned text, agenthub simply stops
// forwarding it. A nil field is left untouched, so blanking a description
// cannot silently drop a rename.
func (h *Hub) SetToolOverride(
	ctx context.Context, server, tool string, override api.ToolOverride, expectedGeneration uint64,
) (api.ToolWrite, error) {
	return call(ctx, h, func(c *api.Client) (api.ToolWrite, error) {
		return c.Tools.SetOverride(ctx, server, tool, override, expectedGeneration)
	})
}

// ListQuarantine returns every isolated tool, keyed by the CLIENT-VISIBLE
// exposed name: that is what an agent could have called.
func (h *Hub) ListQuarantine(ctx context.Context) (api.QuarantineList, error) {
	return call(ctx, h, func(c *api.Client) (api.QuarantineList, error) {
		return c.Quarantine.List(ctx)
	})
}

// ReleaseQuarantine un-isolates one entry. An exposed name that is not in
// the set answers 404 rather than a cheerful success — a typo must not
// report "released" while the real quarantine stands.
func (h *Hub) ReleaseQuarantine(
	ctx context.Context, exposed string, expectedGeneration uint64,
) (api.QuarantineRelease, error) {
	return call(ctx, h, func(c *api.Client) (api.QuarantineRelease, error) {
		return c.Quarantine.Release(ctx, exposed, expectedGeneration)
	})
}
