...
// assembled product — the same distinction each subsystem file's current
// assembly status draws for its own package: shaping's FileStore in
// docs/subsystems/shaping.md#current-assembly-status, discovery's intent
// variants in docs/subsystems/execution.md#current-assembly-status. There is
// no central list to consult, which is the reason each one is written down
// where the code is rather than assumed. Say so here rather than naming consumers as though they were
