...
| 运维 | `agenthub doctor` 全面体检、`agenthub audit` 背后加密且有硬边界的 tools/call 历史、`agenthub logs` 把各进程日志归并成一条时间线、`agenthub events` 以闭集词汇记录 server/gateway/daemon 的状态变更、每 server 的 JSON-RPC 报文抓取（默认关，`server trace`）、X-Request-Id 全链路 |
