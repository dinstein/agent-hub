...
`RemoveServer` 是这条规则铺得最宽的一处：它带走整个痕迹——凭据、profile 成员资格与选择器、
点名该 server 的治理规则、integrity pin、审批记录与授权、quarantine 条目、tool override、
缓存的工具目录。日志是刻意的例外；一份会遗忘已删除 server 的日志作为证据毫无价值。

它做到这个程度的理由**不是**悬空引用危险——那些引用解析为空集，本身就是 fail-closed。
理由是上面这些存储全部**按 server id 索引**，所以重新添加同一个 id 会继承它们：
一条 integrity 基线、一个「永久允许」授权，或一份由完全不同的 server 挣来的 refresh token。
悬空的引用是惰性的，悬空的授权是活的权限。改写引用之所以正当，只因为每一处改写都是
**只收窄**的——`Profile.Servers` 是三态白名单，所以被清空后保持 `[]`，绝不塌缩成 `nil`
（那会把「一个都不给」翻转成「全给」）。带排除语义的字段绝不能在这里被改写。
失败方向：先提交注册表事务，随后每项清理各自独立运行，失败只报 **warning** 并点名什么残留了
——钥匙串或状态文件锁着，绝不能让一台 server 变得删不掉。
