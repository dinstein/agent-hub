...
**`pid` 在每一条网关记录上都是必须的，而且是在构造 logger 时挂一次**、不是每个调用点挂一次——一个进程
的每一行携带的都是同一个值，而在每个调用点去盖章正是某一行最后漏掉它的原因。它存在的理由是：日志**文
件**是按**客户端**命名的，不是按进程——每一次 `agenthub connect --client claude-code` 都往
`gateway-claude-code.log` 追加，而用户手上通常同时跑着好几个，一个编辑器窗口一个。没有它，两个网关交错
的行读起来就像一个网关在做不可能的事：一台 server 在同一瞬间既连上又失败，一道退避阶梯看上去跳过了档
位。这个值就是 `ps` 打印的那个，所以一行日志能直接 join 到机器上的一个进程。

**装不进行长上限的记录会变成 `OversizeMarker`，而每一个读者都必须认得这个形状。** 上限是 4096 =
PIPE_BUF，正是它让一个追加者的行相对另外 N 个是原子的；超过它的记录会被**替换**而不是写出去，因为一个
有界的标记胜过一行撕裂的、会把整条流对所有消费方都弄坏的日志。这个标记和真记录共用 `ts` 字段，所以一个
不先检查 `oversize` 的读者会把它无错地反序列化进自己的类型、拿到一个零值——一行什么都没断言的空记录，
顶替掉那条大到被丢弃的记录。`audit.DecodeOversize` 就是这道检查；`server logs` 在解帧之前跑它，然后渲染
出「帧被丢弃」、尺寸、以及从前缀里捞回来的 method。

**写入方要装下的是**序列化后的行**，不是原始载荷。** 把 body 截到 N 个原始字节，对那一行的长度什么都没
说：body 进的是一个 JSON 字符串，里面引号和反斜杠翻倍、控制字节变六倍。`internal/downstream` 的 trace 日
志曾把这两个数都设成 4096，于是丢掉了每一个载荷超过约 2 KB 的、引号密集的 JSON 帧——一次大的
`tools/list`、一个大的 `tools/call` 结果，也就是**恰恰是**一个人打开 trace 想看的东西。它现在会序列化、
量长度、再重切，直到那一行装得下，得到的是真正写得出去的最大 body，而且完全不产生标记。反过来去调高上
限，只是拿丢掉的帧换来撕裂的帧。

按 server 的 trace 日志（`internal/downstream`，`TraceFrame`）出于同样的理由、在低一层携带同一个字段，
旁边还有 `inst`：那个文件是按 **server** 命名的，所以每一个 trace 它的网关进程都往那里写，而在一个进程
内部每一个派生实例也都往那里写。`server logs` 把 `pid` 渲染成一列，`--json` 里两个都带；在这个字段存在
之前写下的帧显示 `-`，而不是一个读起来像进程号的零。
