...
**令牌缓存是每连接一份的，而且绝不能比 vault 里的事实活得更久。** 凭据只读一次，然后跟着 round tripper
活一辈子——否则 macOS 上每一个请求都要走一趟 keychain。但写这个 vault 的是**别的进程**：`agenthub auth
login`、以及守护进程里那个主动刷新器，它们谁都没有握着某个活 round tripper 的句柄。registry 的热重载面
也帮不上忙：`specEqual` 比的是 URL/args/env/headers，凭据对它是不可见的——一次 vault 写入产生不了 diff，
也就不会重连。而这是**刻意**的：把凭据放进 registry 的比较面，等于让 registry 持有机密。所以这份缓存有
三条规则，每条都由自己的测试钉住：

- **miss 绝不缓存。** 只有命中才置 `loaded`。一台在自己的凭据存在**之前**就被启用的 server
  （`server add` → `server enable` → `auth login`）否则会把空串一直攥到死。
- **被拒绝之后先重读，再走刷新。** 401/403 之后先回 vault 重读一次，因为最常见的成因就是手里这份是旧
  的。重读既不烧 refresh token 也不打扰任何人，所以先试它；`tok != stale` 这道守卫是防止它吞掉真正续期
  路径的——重读回来还是那个被拒绝的值，本身就证明了 vault 里没有更新的东西，于是 OAuth 授权照跑。
- **凭据 epoch 一动，缓存就作废。** 前两条都是**被动**的：它们都需要先有一个请求被拒绝。而一份在手里这
  份还被接受的时候就被**轮转**掉的凭据，根本产生不了任何拒绝，守护进程的刷新器也就永远送不到活连接手
  上。下面这个宣告面补上了这个缺失的信号，`WithEpoch` 是一个 source 主动加入它的方式；没有 epoch 的
  source 完全保持原来那份被动契约。

**宣告面**（`internal/secrets/announce.go`）是 vault 手里那个对应 registry 修订计数器的东西。
`<data>/secrets/credentials.rev` 只记 server id 和一个单调计数器，**别的什么都不记**——这正是它能不加密地
躺在 `secrets.enc` 旁边的原因，并且有测试断言任何一份存下的凭据都不会出现在里面。它是一个独立文件，而
不是对 vault 存储的一次 watch，因为凭据可能住在操作系统钥匙串里，那里原地替换一个值根本不改动任何文件
——而这恰恰是最常见的那一种情况：一次令牌刷新。

`Chain.Set` / `Chain.Delete` 负责宣告，因为它们是每一份凭据都必经的那个咽喉：`auth login`、`secret set`、
守护进程的刷新器最后都落到那里，所以没有哪个调用方能忘掉这件事。两半的失败方向都是 fail-soft——写不出
宣告，或者 `credentials.rev` 解析不了，都退化成上一个版本原本的行为，因为每一个消费方手里都还留着它一
直就有的那条被动路径。

网关拿到一次宣告之后做什么，取决于那台 server 的状态（`internal/gateway/credwatch.go`）：**已连接** → 把
epoch 加一，于是下一个请求会重读 vault，而**什么都不会重连**（守护进程每 60 秒就重写一次 vault，一次刷
新就重连一次是风暴，不是修复）；**未连接** → 唤醒它的重拨档位，好让一次修好了被拒握手的登录，不必再去
熬完一个在这份凭据存在**之前**就挣来的退避。epoch 按 server 而不是按 scope 键控，因为派生实例继承的是基
准 server 的那次登录，一个计数器必须能让全部实例一起失效。

这一整套要防的失败是最难查的那一类：网关把 server 列出来了，而对它的每一个调用都 401，直到客户端重启为
止。注意有一件事**单靠自己关不掉**这个口子——401 重试挂在活连接的 round tripper 上，所以它修得了在一条
可用连接底下过期的凭据，却对一次从来没完成过的握手无能为力。那种情况要靠重拨阶梯（下面的
`internal/gateway`），而宣告面是让它从「早晚会好」变成「立刻就好」的那样东西。
...
一次失败的 dial 会记下**为什么**失败（`connErr`），好让这台 server 报成出错、而不是永远在「连接中」——
而在这个面存在之前，事情到此为止：这条连接再也不会被尝试第二次，于是不论成因是什么，每一次恢复都要付
出一次客户端重启。server 起得比网关慢、网络眨了一下眼、stdio 子进程第一次启动就崩了、或者凭据是在 401
已经被答复之后才到的。

`redial.go` 给每台失败的 server 一道阶梯：**5s、15s、45s、135s，之后永远 5 分钟**，由那次记下的失败来
上膛、由一次成功来清空，所以 `connErr` 和这道阶梯不可能对「这台 server 是不是坏的」给出两种说法。只有基
数可配（`Config.RedialBase`），tick 和上限都从它推导，因为两个独立旋钮会让调用方把基数设到自己的上限之
上。

三条性质是承重的：

- **上限不是装饰。** 没有它，一台真的死掉的 server 会以基数间隔被拨到进程结束为止；而对 stdio 条目来
  说，每一个档位都是一次进程 spawn。
- **驱动阶梯的是那次记下的失败，绝不是 tick。** 已连接的 server 永远不会被重拨；一个按定时器重启健康
  stdio 子进程的网关，比它要修的那个 bug 更糟。
- **dial 按 server 认领**（`beginDial` / `finishDial`），三条路径——启动、热重载、重拨——共用同一个槽位，
  所以一次落在到期档位旁边的重载不可能给同一台 server 造出两条连接。认领不到槽位的那次重载会把这台
  server 交给阶梯，而不是把它丢掉；否则一次在旧定义的 dial 还在飞的时候到达的重定义，最后会没有任何人
  去拨它——因为那次 dial 落地时会发现自己已经过时，然后自己把自己丢掉。

发现模式排除了更省事的那种设计。lazy 模式下，一台失败的 server 的工具根本不在目录里，所以永远不会有调
用到达、去触发一次按需 dial：恢复只能由定时器驱动，不能由流量驱动。
