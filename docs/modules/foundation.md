...
- **A stream's segment list stops at that stream, and the filter compares the STEM.** The glob
  ...
  matches `gateway-claude-code-dev.log` — a second client's ACTIVE log — and everything rotated off
  it. Two clients named `claude-code` and `claude-code-dev` are the whole precondition, and process
  logs are named `gateway-<client>.log` from ids an operator chooses. `Segments` therefore keeps only
  files whose `-<stamp>.p<pid>` suffix was appended to this stream's own base. **`IsSegment` alone is
  not that test**: `gateway-claude-code-dev-<stamp>.p9.log` is a genuine segment, of a different
  stream, and matches the shorter stream's glob — only the stem says which stream a file belongs to.
  Unfiltered, a reader merged a sibling's records into this stream while the sibling's own read
  stayed clean; and since `Prune` walks the same list and `NewWriter` calls `Prune` on every open,
  starting one client's gateway could DELETE another client's live log.
  `TestPruneNeverRemovesASiblingStream` is that case.
...
- **A frame never blocks its caller.** `AppendFrame` queues and returns; an overflow is counted
  ...
  that way.
  - **The tallies are atomics and `frames.mu` guards the file alone.** Counting a drop used to take
    that mutex, which `writerFor` holds across `EnsureDir` and `OpenFile`: a caller whose frame
    arrived with the queue full, while the writer goroutine was opening the next UTC day's file,
    waited on the file system inside the call it was promised never to delay. Both conditions were
    needed and the second happens once per process per day, so the window was narrow — but widest on
    exactly the slow disk that fills the queue in the first place. Anything added to this struct that
    a CALLER touches belongs off `mu` for the same reason.
