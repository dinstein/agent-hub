...
| Cross-process file locks | `LockFileEx`/`UnlockFileEx` on `kernel32.dll` via `syscall.NewLazyDLL`; lock byte at 1<<62 (past any plausible file length, because Windows locks are **mandatory** — locking over data would break reads). Used in `internal/jsonl`, `internal/ratelimit`, `internal/registry`, `internal/skills`, `internal/httpbridge`, `internal/integrity`, `internal/oauthflow` | `test/buildrules/flockparity_test.go`; lock byte is a protocol between processes and must not move |
